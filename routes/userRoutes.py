...
@user_router.put("/makananmu")
def generate_food_recommendation(request: AnswerSchema, db: Session = Depends(get_db), user: str = Depends(authenticate)) -> dict:
    return userControllers.generateFoodRecommendation(request, db, user)